...
#if defined(KLEE_BACKEND) && !OLD_SYMBOLIC_MEMORY
#include "klee_backend/Shadow.h"
#else
...
template <typename T> void concretize(T *addr, size_t nbytes) {
  // Fast path for allocations within one page.
  auto byteBuf = reinterpret_cast<uintptr_t>(addr);
  if (pageStart(byteBuf) == pageStart(byteBuf + nbytes) &&
      !g_shadow_pages.count(pageStart(byteBuf)))
  ...
  WriteShadowIterator shadow((uintptr_t)addr);
  for (int i = 0; i < nbytes; i++, ++shadow) shadow.writeByte(nullptr);
}
